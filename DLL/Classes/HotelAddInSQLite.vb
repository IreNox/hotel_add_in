...
#Region "Init"
    Public Sub New()
        Try
            path = Settings.PathSQLite

            dbConn = New SQLiteConnection("Data Source=" + path + ";Read Only=True;")
            dbConn.Open()
...
#Region "Private Member"
    ...
    Public Function CreateObjectSql(ByVal where As String) As DataTable
...
#Region "Anzahl"
    Public Function RsAnzahlGästeImHaus(ByVal Datum As Date) As Integer
        ...
        Dim table = Me.CreateObjectSql("date([BU_DATUM_VON]) <= date('" + Datum.ToString("yyyy-MM-dd") + "') AND date([BU_DATUM_BIS]) > date('" + Datum.ToString("yyyy-MM-dd") + "')")
